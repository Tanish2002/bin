...
if [ $WAYLAND_DISPLAY ]; then
	...
	case $chosen in
         "fullscreen")
             ...
             ;;
         "area")
             sleep 0.5; maim -u -s ~/screenshots/$(date "+%F_%H:%M:%S").png && notify-send '	selected area screenshot saved!' -t 1500 || break
             ;;
         "area [clipboard]")
             sleep 0.5; maim -s | xclip -selection clipboard -t image/png && notify-send '	screenshot copied to clipboard' -t 1500 || break
             ;;
         "active window")
